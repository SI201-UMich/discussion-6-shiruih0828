...
###############################################################################
    ...
    def horse_fastest_race(self, horse):
        ...
        races = self.race_dict[horse]  # dict of race->time
        fastest_race = None
        fastest_time = 999.9

        for race_name, time in races.items():
    ...
    def get_average_time(self):
        ...
        for horse, races in self.race_dict.items():
            ...
            for time in races.values():
                total += time
                count += 1
            # assume each horse has at least one race
            avg[horse] = total / count if count > 0 else 0.0
        return avg
